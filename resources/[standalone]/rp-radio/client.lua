...
Citizen.CreateThread(function ()
 ...
 Citizen.Wait(5000)
 if ESX.PlayerData.job and (ESX.PlayerData.job.name == 'ambulance' or ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'admin' or ESX.PlayerData.job.name == 'council') then 
    exports["rp-radio"]:GivePlayerAccessToFrequencies(1,2,3,4,5,6,7,8,9)
else
...
RegisterCommand('fixbugprop', function ()
    local Coords = GetEntityCoords(PlayerPedId())
    ClearAreaOfObjects(Coords, 2.5)
    for k,v in pairs(PROPS) do
        local prop = GetClosestObjectOfType(Coords, 1.0, GetHashKey(k), false, false, false)
...
function Radio:Toggle(toggle)
    ...
    if not self.Has or IsEntityDead(playerPed) then
        ...
        TriggerServerEvent('bt_attacher:radio:remove')
        return
    end
    ...
    if self.Open then
        ...
        TriggerServerEvent('bt_attacher:radio:add')
    ...
    while true do
      ...
      --  elseif (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has)) or ((not checkHasItem('radio') and isDead) and Radio.On) then
        ...
        if Radio.Open then
            ...
            if Radio.On then
                if exports["esx_policejob"]:handcuff() == false then
...
openwait = false
local calledradio = function()
    if exports["esx_policejob"]:handcuff() == true then return end
   -- local isFalling = IsPedFalling(PlayerPedId())
    local isDead = IsEntityDead(PlayerPedId())
   -- print(shiftStatus and lStatus and not isFalling and Radio.Enabled and Radio.Has and not isDead and checkHasItem('radio') and openwait == false)
    if shiftStatus and lStatus and Radio.Enabled and Radio.Has and not isDead and checkHasItem() and openwait == false then 
       -- print('radio ')
        openwait = true
        Radio:Toggle(true)
        Wait(1000)
...
local pressedShiftedPushed = function()
    shiftStatus = not shiftStatus
    calledradio()
end

local pressedEPush = function()
    lStatus = not lStatus
    calledradio()
end

RegisterCommand('+reisoonl'    , pressedEPush)
RegisterCommand('-reisoonl'    , pressedEPush)
RegisterCommand('+reioonxshift'    , pressedShiftedPushed)
RegisterCommand('-reioonxshift', pressedShiftedPushed)

RegisterKeyMapping('+reioonxshift' , 'Radio(1)'  , 'keyboard'    , 'lshift')
RegisterKeyMapping('+reisoonl'     , 'Radio(2)'      , 'keyboard'    , 'l')
...
AddEventHandler('esx:addInventoryItem', function(item, count)
    ...
    if item == 'radio' or item == 'radio_job' then
        haveitem = true
    end
...
checkHasItem = function()
  ...
  loadcheckitem = function()
    ...
    for i=1, #inventory do
      ...
      if 'radio' == item.name and item.count > 0 or 'radio_job' == item.name and item.count > 0 then
        haveitem = true
        return true
      end
    end
...
AddEventHandler("Radio.Toggle", function()
    ...
    local isDead = IsEntityDead(playerPed)
    if exports["esx_policejob"]:handcuff() == true then return end
    if not isDead and Radio.Enabled and Radio.Has then
        Radio:Toggle(not Radio.Open)
    end
end)
