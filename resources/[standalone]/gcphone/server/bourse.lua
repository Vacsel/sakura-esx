...
ESX.RegisterServerCallback("gcPhone:getCrypto",function(a,b,c)
    ...
    MySQL.query("SELECT crypto FROM users WHERE identifier = @identifier",{["@identifier"]=d.identifier},function(e)
...
AddEventHandler("gcPhone:buyCrypto",function(a,b,c,d)
    ...
    if not e then
         ...
         local g={}
         MySQL.query("SELECT crypto FROM users WHERE identifier = @identifier",{["@identifier"]=e.identifier},function(h)
            if f<0 then f=f*-1 
            end;
            ...
            MySQL.query("UPDATE users SET crypto = @crypto WHERE identifier = @identifier",{["@identifier"]=e.identifier,["@crypto"]=json.encode(g)})
        end)
    end)
