...
ESX.RegisterServerCallback("crew:getBills", function(a, b)
    ...
    MySQL.query("SELECT amount, id, target, label FROM billing WHERE identifier = @identifier", {["@identifier"] = c.identifier}, function(d)
        b(d)
    end)
end)
...
AddEventHandler("gcPhone:faturapayBill", function(a)
    ...
    MySQL.query("SELECT * FROM billing WHERE id = @id", {["@id"] = a}, function(c)
        local d = c[1].sender;
        ...
        if e == "player" then if h ~= nil then
            if b.getAccount("bank").money >= g then
                MySQL.update("DELETE from billing WHERE id = @id", {["@id"] = a}, function(i)
