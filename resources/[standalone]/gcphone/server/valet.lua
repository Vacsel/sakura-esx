...
ESX.RegisterServerCallback("gcPhone:getCars", function(a, b)
    ...
    MySQL.query("SELECT plate, vehicle, stored FROM owned_vehicles WHERE owner = @cid and type = @type", {["@cid"] = c.identifier, ["@type"] = "car"}, function(d)
...
AddEventHandler("gcPhone:valet-car-set-outside", function(a)
    ...
    local c = ESX.GetPlayerFromId(b)
    if c then
        MySQL.insert("UPDATE owned_vehicles SET stored = @stored WHERE plate = @plate", {["@plate"] = a, ["@stored"] = 0})
    end
end)
