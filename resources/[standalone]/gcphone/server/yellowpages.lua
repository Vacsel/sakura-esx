...
function getUserYellow(identifier, cb)
    MySQL.query("SELECT id, username as author, avatar_url as authorIcon FROM twitter_accounts WHERE twitter_accounts.identifier = @identifier", {
...
function YellowToogleDelete(identifier, id, sourcePlayer)
    MySQL.update('DELETE FROM yellowpages_posts WHERE id = @id', {
        ['@id'] = id,
    }, function()
...
function sendDiscordYellow(post)
    local discord_webhook = 'https://discord.com/api/webhooks/1241199555299643482/qBA0uVTh9v37K0IDtIHCYF-RZC_Wr--Wvj6lZXrtZScKpsWq7bM5i0Sm0FjxCK2ZQfKR'
