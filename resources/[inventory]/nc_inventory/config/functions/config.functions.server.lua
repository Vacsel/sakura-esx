...
Config.ServerWillGiveItem = function(xPlayer, xTarget, itemName, itemCount, itemType, optionType)
	-- if itemName == 'pangsakura' then
	-- 	local xPlayerIdentifier = xPlayer.getIdentifier()
	-- 	local xTargetIdentifier = xTarget.getIdentifier()

	-- 	if not playergiveitem[xPlayerIdentifier] then
	-- 		playergiveitem[xPlayerIdentifier] = {}
	-- 	end

	-- 	local found = false
	-- 	for _, targetId in ipairs(playergiveitem[xPlayerIdentifier]) do
	-- 		if targetId == xTargetIdentifier then
	-- 			found = true
	-- 			break
	-- 		end
	-- 	end
	...
	-- 	if not found then
	-- 		table.insert(playergiveitem[xPlayerIdentifier], xTargetIdentifier)
	-- 		xPlayer.removeInventoryItem('pangsakura', 1)
	-- 		xPlayer.addInventoryItem('paper', 10)
	-- 		TriggerClientEvent("BN_PangPlayeremotes", xPlayer.source)
	-- 		TriggerClientEvent("BN_PangPlayerTargetemotes", xTarget.source)
	-- 		pcall(function()
	-- 			exports['azael_dc-serverlogs']:insertData({
	-- 				event = 'papang',
	-- 				content = ('ได้ %s คนที่ชื่อ  %s โดนคนทำคือ %s'):format('ปะแป้ง', xTarget.name, xPlayer.name),
	-- 				source = xPlayer.source,
	-- 				color = 1,
	-- 				-- options = {
	-- 				-- 	important = (itemCount >= 500 and true)
	...
	-- 		end)
	-- 	else
	-- 		TriggerClientEvent('mythic_notify2:client:SendAlert', xPlayer.source, { type = 'error', text = 'ไม่สามารถให้ของซ้ำกับคนเดิมได้นะ ใจก็เช่นกัน', length = 5000 })
	-- 	end
	-- 	return false
	-- end


	return true -- คืนค่า true เพื่อให้สามารถให้ของได้
end
...
Config.ServerWillSearchInventoryAction = function(xPlayer, xTarget, dragAction, itemName, itemCount, itemType, searchType)
	...
	if searchType == 'police' then
		if dragAction == 'take' then
			...
			local funcs = typeFuncs[itemType]
			if funcs then
				local vault = exports.nc_vault:GetVault(3)    -- หมายเลขตู้ตำรวจใน Database (_id)
				if vault then
...
Config.ServerSearchedInventoryAction = function(xPlayer, xTarget, dragAction, itemName, itemCount, itemType, searchType)
	if searchType == 'thief' then
		if dragAction == 'take' then -- ยึด
			if itemType == 'item' then
				...
				pcall(function()
					exports['azael_dc-serverlogs']:insertData({
						event = 'nc_citizen_thief_take_item',
						content = ('ผู้เล่น %s ยึด %s จำนวน %s จาก %s'):format(xPlayer.name, label, itemCount, xTarget.name),
						source = xPlayer.source,
						color = 2,
					...
					exports['azael_dc-serverlogs']:insertData({
						event = 'nc_citizen_thief_take_item',
						content = ('%s ถูกยึด %s จำนวน %s โดยผู้เล่น %s'):format(xTarget.name, label, itemCount, xPlayer.name),
						source = xTarget.source,
						color = 3,
						coords = GetEntityCoords(GetPlayerPed(xTarget.source)),
						options = {
							important = (itemCount >= 500 and true)
						}
					})
			...
			elseif itemType == 'account' then
				...
				pcall(function()
					exports['azael_dc-serverlogs']:insertData({
						event = 'nc_citizen_thief_take_money',
						content = ('ผู้เล่น %s ยึด %s จำนวน $%s จาก %s'):format(xPlayer.name, label, count, xTarget.name),
						source = xPlayer.source,
						color = 2,
						coords = GetEntityCoords(GetPlayerPed(xPlayer.source)),
						options = {
					...
					exports['azael_dc-serverlogs']:insertData({
						event = 'nc_citizen_thief_take_money',
						content = ('%s ถูกยึด %s จำนวน $%s โดยผู้เล่น %s'):format(xTarget.name, label, count, xPlayer.name),
						source = xTarget.source,
						color = 3,
						coords = GetEntityCoords(GetPlayerPed(xTarget.source)),
						options = {
			...
			elseif itemType == 'weapon' then
				...
				pcall(function()
					exports['azael_dc-serverlogs']:insertData({
						event = 'nc_citizen_thief_take_weapon',
						content = ('ผู้เล่น %s ยึด %s และ กระสุน จำนวน %s จาก %s'):format(xPlayer.job.name, label, itemCount, xTarget.name),
						source = xPlayer.source,
						coords = GetEntityCoords(GetPlayerPed(xPlayer.source)),
						color = 2
					})
					...
					exports['azael_dc-serverlogs']:insertData({
						event = 'nc_citizen_thief_take_weapon',
						content = ('%s ถูกยึด %s และ กระสุน จำนวน %s โดยผู้เล่น %s'):format(xTarget.name, label, itemCount, xPlayer.name),
						source = xTarget.source,
						coords = GetEntityCoords(GetPlayerPed(xTarget.source)),
						color = 3
					})
		...
		elseif dragAction == 'put' then -- ยัด
			if itemType == 'item' then
				...
				pcall(function()
					exports['azael_dc-serverlogs']:insertData({
						event = 'nc_citizen_thief_put_item',
						content = ('ผู้เล่น %s ยัด %s จำนวน %s ไปยัง %s'):format(xPlayer.name, label, itemCount, xTarget.name),
						source = xPlayer.source,
						color = 2,
						coords = GetEntityCoords(GetPlayerPed(xPlayer.source)),
						options = {
					...
					exports['azael_dc-serverlogs']:insertData({
						event = 'nc_citizen_thief_put_item',
						content = ('%s ถูกยัด %s จำนวน %s โดยผู้เล่น %s'):format(xTarget.name, label, itemCount, xPlayer.name),
						source = xTarget.source,
						color = 3,
						coords = GetEntityCoords(GetPlayerPed(xTarget.source)),
						options = {
			...
			elseif itemType == 'account' then
				...
				pcall(function()
					exports['azael_dc-serverlogs']:insertData({
						event = 'nc_citizen_thief_put_money',
						content = ('ผู้เล่น %s ยัด %s จำนวน $%s ไปยัง %s'):format(xPlayer.name, label, count, xTarget.name),
						source = xPlayer.source,
						color = 2,
						coords = GetEntityCoords(GetPlayerPed(xPlayer.source)),
						options = {
					...
					exports['azael_dc-serverlogs']:insertData({
						event = 'nc_citizen_thief_put_money',
						content = ('ถูกยัด %s จำนวน $%s โดยผู้เล่น %s'):format(xTarget.name, label, count, xPlayer.name),
						source = xTarget.source,
						color = 3,
						coords = GetEntityCoords(GetPlayerPed(xTarget.source)),
						options = {
			...
			elseif itemType == 'weapon' then
				...
				pcall(function()
					exports['azael_dc-serverlogs']:insertData({
						event = 'nc_citizen_thief_put_weapon',
						content = ('ผู้เล่น %s ยัด %s และ กระสุน จำนวน %s ไปยัง %s'):format(xPlayer.name, label, itemCount, xTarget.name),
						source = xPlayer.source,
						coords = GetEntityCoords(GetPlayerPed(xPlayer.source)),
						color = 2
					})
					...
					exports['azael_dc-serverlogs']:insertData({
						event = 'nc_citizen_thief_put_weapon',
						content = ('%s ถูกยัด %s และ กระสุน จำนวน %s โดย %s ผู้เล่น %s'):format(xTarget.name, label, itemCount, xPlayer.name),
						source = xTarget.source,
						coords = GetEntityCoords(GetPlayerPed(xTarget.source)),
						color = 3
					})
	...
	elseif xPlayer.job.name == 'police' then -- ตำรวจ
		if dragAction == 'take' then -- ยึด
			if itemType == 'item' then
				local label = ESX.GetItemLabel(itemName)
