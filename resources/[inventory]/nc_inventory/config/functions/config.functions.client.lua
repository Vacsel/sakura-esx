...
Config.ClientWillOpenInventory = function(secondaryName)
	if not IsScreenFadedOut() and not IsPauseMenuActive() then
		if ESX.GetPlayerData().IsBlockAny == nil then
...
Config.ClientOpenedInventory = function(secondaryName)
	TriggerEvent('nextra_inve_bridge:openInventory')
...
Config.ClientClosedInventory = function(secondaryName)
	...
	TriggerEvent('nextra_inve_bridge:closeInventory')
	ExecuteCommand('toggleUI')
end
local lastweapon = -1569615261
...
Config.ClientWillUseItem = function(itemName, itemType, shortcut)
	...
	if itemName == 'armor_job' then
		if PlayerData.job.name == 'offpolice' or PlayerData.job.name == 'offambulance' or PlayerData.job.name == 'offcouncil' then
			return false
		end
	end
	if itemType == 'vehicle_key' then
		--pcall(function() exports.nc_vehiclekey:ToggleLockVehicle(itemName) end)
		pcall(function() exports["BN_Vehiclelock"]:Togglecar(itemName) end)

		return false
	end
	if itemName == 'id_card' then
		TriggerEvent('jsfour-idcard:open1')
		return true
	end

	if itemType == 'weapon' and GetHashKey(itemName) == lastweapon then
	
		local animDict = 'reaction@intimidation@1h'
		local animOutroName = 'outro'
		local playerPed = PlayerPedId()
		itemaction = true
		TriggerEvent('Black_Antiblock:blockswitch', true)
		SetCurrentPedWeapon(playerPed, GetHashKey(itemName), true)
		ESX.Streaming.RequestAnimDict(animDict, function()
			TaskPlayAnim(playerPed, animDict, animOutroName, 8.0, -8.0, 2100, 48, 0.0, false, false, false)
		end)
		Citizen.Wait(1000) -- Wait for animation to finish

		-- Ensure the weapon remains unarmed
		SetCurrentPedWeapon(playerPed, -1569615261, true)
		lastweapon = -1569615261
		TriggerServerEvent('debug_weapon:updateCurrentWeapon', itemName)
		Citizen.Wait(2000)
		TriggerEvent('Black_Antiblock:blockswitch', false)
		itemaction = false
		
		return false
	end
	if itemaction and itemType == 'weapon' then
		return false
	end
	-- if checkblockcloth == true then
...
Config.ClientUsedItem = function(itemName, itemType, shortcut)
	if itemName == 'armor' then
		pcall(function()
			exports['xcore-quest']:addQuest("armor", 1)
		end)
		pcall(function()
			exports['xcore-quest']:addQuest("armor50", 1)
		end)
	end
	...
	if itemType == 'weapon' then
		...
		local animFlag = 0
		local playerPed = PlayerPedId()
		local heaveitem = GetSelectedPedWeapon(PlayerPedId())
		itemaction = true
		TriggerEvent('Black_Antiblock:blockswitch', true)
		--print(heaveitem , lastweapon)
		if heaveitem ~= lastweapon then
				
			SetCurrentPedWeapon(playerPed, lastweapon, true)
			ESX.Streaming.RequestAnimDict(animDict, function()
				TaskPlayAnim(playerPed, animDict, animIntroName, 8.0, -8.0, 2700, 48, 0.0, false, false, false)
			end)
			
			Citizen.Wait(1000) -- Wait for animation to finish
			-- Switch back to the selected weapon
			SetCurrentPedWeapon(playerPed, heaveitem, true)
			TriggerServerEvent('debug_weapon:updateCurrentWeapon', itemName)
			lastweapon = heaveitem
			Citizen.Wait(2000)
...
Config.ClientWillDropItem = function(itemName, itemLabel, itemCount, itemType, optionType)
	...
	if Allowitem[itemName] then
		if IsDrop then
			TriggerEvent('mythic_notify2:client:SendAlert', { type = 'success', text = 'คุณกำลังทำสิ่งนี้อยู่ กรุณารอสักครู่', length = 2*1000 })
			return false
		end
		IsDrop = true
		TriggerEvent('mythic_notify2:client:SendAlert', { type = 'success', text = 'คุณกำลังได้ทิ้ง '..itemLabel..' จำนวน '..itemCount, length = 60000 })
		Wait(60000)
		IsDrop = false
		return true
	else
...
Config.ClientSecondaryInventoryAction = function(secondaryName, dragAction, item, job)
	if secondaryName == 'vault' or secondaryName == 'house_vault' or secondaryName == 'trunk' then
...
Config.ClientWillDoAnimationOnUseWeapon = function(animationType, weaponHash, swapWeaponHash)
...
-- animationType จะมี 2 ค่าคือ 'put' => เก็บอาวุธ | 'take' => หยิบอาวุธ
Config.ClientDoneAnimationOnUseWeapon = function(animationType, weaponHash, swapWeaponHash)
...
-- target_not_nearby				=>	ผู้เล่นมอบของให้ผู้อื่น - ไม่พบบุคคลนี้ในบริเวณใกล้เคียง
...
Config.OnError = function(errAction, errType, errText, itemName)
	...
	-- target_block_giving				=>	'error' - คนๆนี้ไม่ต้องการรับของจากผู้อื่น
	-- target_not_nearby				=>	'error' - ไม่พบบุคคลนี้ในบริเวณใกล้เคียง
...
RegisterNUICallback('getWeaponSkins', function(data, cb)
	cb(true)
	
	local data = exports['BN_weaponskin']:OpenWeaponSkin(data.weaponName)
	SendNUIMessage({
		action = 'openWeaponSkinMenu',
		data = data
	})
end)

RegisterNUICallback('handleSelectedSkin', function(data, cb)
	exports['BN_weaponskin']:HandleWeaponSkin(data)
end)